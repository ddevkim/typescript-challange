...
type StrToNum<S extends string> = S extends `${infer A extends number}`
  ? A
  : never;

type GetLastStr<S extends string> = S extends `${infer A}${infer B}`
  ? B extends ""
    ? A
    : GetLastStr<B>
  : S;

// type GG = ReverseString<"192300">

type MyReverseStr<S extends string> = S extends `${infer A}${infer Rest}`
  ? `${MyReverseStr<Rest>}${A}`
  : S;

// type MinuseOne<N extends number> = MyReverseStr<NumToStr<N>>

// type Result = MinuseOne<123>
